...
(function(indexJs) {
	...
	$("#onDuty").click(function() {
		$.ajax({
			...
			success : function(data) {
				if (data.ondutymap.status.match('success')) {
	...
	$("#offDuty").click(function() {
		$.ajax({
			...
			success : function(data) {
				if (data.offdutymap.status.match('success')) {
					indexJs.calendar();
				} else {
					alert("打卡失敗!");
				}
	...
	$("#logOut").click(function() {
		$.ajax({
			...
			success : function(data) {
				alert("登出");
				if (data.logoutmap.status.match('success')) {
...
//	});
	...
	indexJs.calendar = function() {
		...
		data : {
		  ...
		  success: function(data) {
			  var PRs=data.recordmap.listPRs;
			  var ARs=data.recordmap.listARs;
			  var arr=[];
...
//			alert("行事曆"+xhr.status);
		...
		indexJs.getApplyTypeList = function() {
			$.ajax({
			url : "getApplyType.action", // 存取Json的網址
			type : "POST",
			cache : false,
			dataType : 'json',
			data : {},
			success: function(data) {
				var doc=data.typemap.listAT;
				var str ='' ;
				$.each(doc, function(i,val){
					var at_no=val.at_no;
